 ...
 * 
 * 由于java中的泛型在编译后被擦除，为防止将List<String>拷贝到List<Double>之类的乌龙，Collection的任何实现类均视为类型不同，不予拷贝。
 * 
 * @author liuyu
 *
 * @param <A>
...
//		classB = (Class<?>) types[1];
		...
		for(Field fa:fieeldsA){
			for(Field fb:fieeldsB){
				if(fa.getName().equals(fb.getName())  && fa.getType().equals(fb.getType()) && !Collection.class.isAssignableFrom(fa.getType())){
